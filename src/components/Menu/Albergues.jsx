...
export const Albergues = () => {
  ...
  const handleInputChange = (name, value) => {
    const newData = editingAlbergue
      ? { ...editingAlbergue, [name]: value }
      : { ...newAlbergue, [name]: value };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingAlbergue) {
        ...
        setAlbergues(
          albergues.map((a) =>
            a._id === id ? { ...a, ...response.data.albergue } : a
          )
        );
  ...
  const handleInspectQR = useCallback(async (id) => {
    try {
      const token = localStorage.getItem("token");
    ...
    <div className="p-4">
        ...
        {filteredAlbergues.map((albergue) => (
          <Card
            ...
            items={[
              {
                icon: FaUsers,
                text: `Población: ${albergue.ciudadanosCount} / ${albergue.capacidadCiudadanos}`,
                color: "blue",
              },
              {
                icon: FaWarehouse,
                text: `Bodegas: ${albergue.bodegasCount} / ${albergue.capacidadBodegas}`,
                color: "green",
              },
              {
                icon: FaUserTie,
                text: `Administradores: ${albergue.usuariosCount} / ${albergue.capacidadUsuarios}`,
                color: "purple",
              },
              {
                icon: FaMapMarkerAlt,
                text: `Ubicación: ${albergue.cordenadas_x}, ${albergue.cordenadas_y}`,
                color: "red",
              },
            ]}
            actions={[
              ...
              {
                icon: FaEdit,
                onClick: () => handleEdit(albergue),
                color: "blue",
                type: "edit",
              },
              {
                icon: FaTrash,
                onClick: () => handleDelete(albergue._id),
                color: "red",
                type: "delete",
              },
            ]}
      ...
      >
        ...
        <GenericInput
          ...
          value={
            editingAlbergue
              ? editingAlbergue.capacidadCiudadanos
              : newAlbergue.capacidadCiudadanos
          }
          onChange={handleInputChange}
        ...
        <GenericInput
          ...
          value={
            editingAlbergue
              ? editingAlbergue.capacidadBodegas
              : newAlbergue.capacidadBodegas
          }
          onChange={handleInputChange}
        ...
        <GenericInput
          ...
          value={
            editingAlbergue
              ? editingAlbergue.capacidadUsuarios
              : newAlbergue.capacidadUsuarios
          }
          onChange={handleInputChange}
