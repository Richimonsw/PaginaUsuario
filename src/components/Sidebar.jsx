  ...
  return (
    <>
      <div className="md:hidden fixed p-2 z-50">
        <button onClick={toggleSidebar} className="text-white bg-indigo-700 p-2 rounded focus:outline-none">
          {isOpen ? <FaTimes size={24} /> : <FaBars size={24} />}
        </button>
      </div>
      <div className={`fixed md:static top-0 left-0 h-full bg-indigo-700 text-white transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 w-64 z-40 shadow-xl`}>
        <div className="flex items-center justify-center mt-10">
          <div className="flex flex-col items-center mb-3 ">
            ...
            <div className="mt-4 text-center">
              <h2 className="text-xl font-semibold text-gray-100">Administrador </h2>
            </div>
          </div>
        </div>
